...
public:
    ...
    void setBackgroundImage(const QString &bgImagePath) {
        ...
        update(); // trigger repaint so the background refreshes
    }
...
int main(int argc, char *argv[]) {
    ...
    controlRow->addSpacing(16);  // left padding for the volume slider
    controlRow->addWidget(volumeSlider, 1);  // keep the slider centered with stretch factor 1
    controlRow->addSpacing(16);  // right padding for the volume slider
    ...
    QObject::connect(toggleCommentButton, &QCheckBox::toggled,
                     [commentPanel, commentPlaceholder, berealCard](bool checked) {
                        if (checked) {
                             // show the interaction stack again
                             commentPlaceholder->hide();
                             commentPanel->show();
                         } else {
                             // hide interactions but keep the layout height stable
                             // measure the panel before hiding to preserve size
                             int panelHeight = commentPanel->isVisible() 
                                 ? commentPanel->height() 
                                 : commentPanel->sizeHint().height();
                             if (panelHeight <= 0) {
                                 // fall back to a safe default height
                                 panelHeight = 120;
                             }
                             // assign the placeholder height so spacing stays consistent
                             commentPlaceholder->setFixedHeight(panelHeight);
                             commentPlaceholder->show();
                             commentPanel->hide();
                         }
                        // request a geometry update without disturbing scroll state
                         if (berealCard) {
    ...
    auto updatePlayPauseVisual = [playPauseButton, player]() {
        ...
        // the night theme defaults to high-contrast *1.svg icons
    ...
    auto initSharePanel = [&]() {
        if (!panelShare) {
            ...
            // add confirm button
            shareConfirmButton = new QPushButton(container);
            shareConfirmButton->setText(pickText(QString::fromUtf8("确定"), QStringLiteral("Confirm")));
    ...
    window.setWindowTitle("Antipode");
